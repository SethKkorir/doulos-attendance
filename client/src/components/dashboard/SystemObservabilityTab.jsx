...
const SystemObservabilityTab = ({ 
    ...
    // --- ACTION HANDLER: BLOCK ATTENDEE ---
    const handleBlockAttendee = async (errId, studentName, studentRegNo, campus) => {
        if (isGuest) {
            setMsg({ type: 'error', text: 'Blocking students is disabled in Guest Mode.' });
            return;
        }

        if (!studentRegNo || studentRegNo === 'UNKNOWN') {
            setMsg({ type: 'error', text: 'Cannot block a student with UNKNOWN registration number.' });
            return;
        }

        setActionLoading(prev => ({ ...prev, [`block-${errId}`]: true }));

        try {
            await api.post('/members/block-by-reg', {
                studentRegNo,
                name: studentName,
                campus
            });

            setMsg({ type: 'success', text: `Success! Student ${studentRegNo} has been blocked.` });
            ...
            // Fetch telemetry after a small delay
            setTimeout(fetchTelemetry, 600);

        } catch (err) {
            setMsg({ type: 'error', text: `Failed to block student: ${err.response?.data?.message || 'Server error'}` });
        } finally {
            setActionLoading(prev => ({ ...prev, [`block-${errId}`]: false }));
        }
    };
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
                        ...
                        <div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                ...
                                <span style={{ background: hasActiveSession ? 'rgba(34, 197, 94, 0.08)' : 'rgba(239, 68, 68, 0.08)', color: hasActiveSession ? '#4ade80' : '#f87171', border: `1px solid ${hasActiveSession ? 'rgba(34, 197, 94, 0.15)' : 'rgba(239, 68, 68, 0.15)'}`, fontSize: '0.55rem', fontWeight: 900, padding: '0.15rem 0.45rem', borderRadius: '1rem', display: 'flex', alignItems: 'center', gap: '3px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
            ...
            {hasActiveSession ? (
                            ...
                            <div style={{ 
                                ...
                                {activities.length === 0 ? (
                                    <div className="glass-card-premium" style={{ 
                                        ...
                                        No check-ins registered in this session yet.
                                    </div>
                                ) : (
                                    activities.map((act) => (
                                        <div 
                                        ...
                                        >
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                                ...
                                                <button 
                                                    ...
                                                    onClick={() => handleBlockAttendee(err.id, err.name, err.regNo, err.campus)}
                                                    disabled={actionLoading[`block-${err.id}`]}
                                                >
                                                    <AlertOctagon size={13} /> {actionLoading[`block-${err.id}`] ? 'BLOCKING...' : '🚫 BLOCK ATTENDEE'}
                                                </button>
                                            </div>
                                        </div>
                                    ))
                ...
                <div className="glass-card-premium" style={{ 
                    ...
                    <div style={{
                        ...
                        justifyContent: 'center',
                        color: '#25AAE1',
                        boxShadow: '0 0 20px rgba(29, 166, 217, 0.1)'
                    }}>
                        <Terminal size={40} />
                    </div>
                    <div>
